...
const translations = {
  ru: {
    ...
    service1Text: "Организация грузовых авиаперевозок по всему миру с оптимальными тарифами.",
    ...
    formFrom: "Откуда *",
    formTo: "Куда *",
    chooseTransport: "Выберите транспорт *",
    cargoType: "Тип груза *",
    formWeight: "Вес (кг)",
    formSize: "Размер (см)",
    ...
    // Selects
    transportOptions: ["✈ Авиа", "🚚 Авто", "🚆 Ж/д", "🚢 Морской"],
    cargoOptions: ["📦 Общий груз", "⚠ Опасный груз", "🔹 Хрупкий груз", "💧 Жидкости", "🥫 Продукты питания", "➕ Другое"]
  ...
  en: {
    ...
    service1: "Air Cargo",
    service1Text: "Organization of air cargo transportation worldwide with optimal rates.",
    ...
    formFrom: "From *",
    formTo: "To *",
    chooseTransport: "Select transport *",
    ...
    // Selects
    transportOptions: ["✈ Air", "🚚 Road", "🚆 Rail", "🚢 Sea"],
    cargoOptions: ["📦 General Cargo", "⚠ Dangerous Goods", "🔹 Fragile Cargo", "💧 Liquids", "🥫 Food Products", "➕ Other"]
  ...
  uz: {
    ...
    service1Text: "Butun dunyo bo‘ylab optimal tariflar bilan yuk tashish.",
    ...
    formFrom: "Qayerdan *",
    formTo: "Qayerga *",
    chooseTransport: "Transportni tanlang *",
    cargoType: "Yuk turi *",
    ...
    // Selects  
    transportOptions: ["✈ Havo", "🚚 Avto", "🚆 Temir yo‘l", "🚢 Dengiz"],
    cargoOptions: ["📦 Umumiy yuk", "⚠ Xavfli yuk", "🔹 Mo‘rt yuk", "💧 Suyuqliklar", "🥫 Oziq-ovqat mahsulotlari", "➕ Boshqa"]
...
function updateLanguage(lang) {
  ...
  const placeholderIds = [
    "formName","formPhone","formFrom","formTo",
    "formWeight","formSize","formProduct","formEmail"
  ];
  ...
  if (transport && translations[lang].transportOptions) {
    transport.options[0].text = translations[lang].chooseTransport;
    for (let i = 1; i < transport.options.length; i++) {
      transport.options[i].text = translations[lang].transportOptions[i - 1];
    }
  }

  const cargo = document.getElementById("formCargo");
  if (cargo && translations[lang].cargoOptions) {
    cargo.options[0].text = translations[lang].cargoType;
    for (let i = 1; i < cargo.options.length; i++) {
      cargo.options[i].text = translations[lang].cargoOptions[i - 1];
    }
  }
}
